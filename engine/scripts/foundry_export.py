...
The store under `WHEATEAR_FOUNDRY_HOME` is the machine's copy: JSON keyed by
...
store does not hold -- the source records actually converted to IR.
    ...
    <out>/ir/                          source records converted to Wheatear IR
...
from wheatear.foundry import runtime  # noqa: E402
from wheatear.foundry.store import FoundryStore  # noqa: E402
from wheatear.foundry.types import Direction, SchemaCorpus  # noqa: E402
...
def mapping_table(store: FoundryStore, adapters: list) -> str:
    ...
    lines = [
        "# Field mappings",
        "",
        "One section per adapter. `import` maps a platform record onto Wheatear's IR;",
...
def main() -> None:
    store = FoundryStore(Path(os.environ["WHEATEAR_FOUNDRY_HOME"]))
    ...
    body = ["# Wheatear foundry -- probe and mapping output", ""]
