...
shape per entity kind. That means a platform Wheatear has never seen still
...
#
# Two kinds: packaging the vendor wraps a solution in, and Wheatear's own
...
def scan_export(path: Path) -> ProbeResult:
    ...
    if path.is_file() and path.suffix.lower() == ".zip":
        with tempfile.TemporaryDirectory(prefix="wheatear-scan-") as tmp:
