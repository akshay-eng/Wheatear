...
def match_tools(export_dir, bot_schema, url_env, key_env, llm_provider, no_llm, top, cookie_env, no_catalog):
    ...
    involved.
    """
    from wheatear import creds as _creds
    from wheatear.config import load_config
    from wheatear.connectors.copilot_studio import solution_importer
    from wheatear.connectors.orchestrate.rest_client import OrchestrateRestClient
    from wheatear.pipeline.resolve import build_catalog, resolve_agent_tools, shortlist_scored
    ...
    if not no_llm:
        ...
        if not key:
            raise click.ClickException(
                "No LLM key found. Run `wheatear wizard` once to store one, or pass --no-llm."
            )
