...
def test_save_config_never_writes_a_secret_value(tmp_path):
    ...
    assert set(raw.keys()) == {
        ...
        "foundry_store_root",         # a local filesystem path, not a secret
        "n8n_base_url",               # n8n instance URL, not a secret
        "n8n_api_key_env",            # stores env-var name, never the key value
