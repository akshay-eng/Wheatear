"""Canonical intermediate representation (IR) for Agent Liftoff.
...
class DialogNodeKind(str, Enum):
    """A constrained subset of dialog-graph node types Agent Liftoff understands.
...
class KnowledgeRef(BaseModel):
    ...
    # separate from `notes`: notes are Agent Liftoff's commentary on the migration,
...
class ConnectionRef(BaseModel):
    ...
    Agent Liftoff; review_required defaults to True because a human always has to
