...
familiar CLI pattern; the colors are Agent Liftoff's own -- Waypoint Amber and
...
def _logo_lockup(console_width: int) -> Table | Text:
    # Two words, not "AGENT_LIFTOFF": pyfiglet stacks on the space, giving a
    # 53-column wordmark instead of a 77-column one. On an 80-column terminal
    # the single-line form plus the mark overflows and rich crops it mid-glyph.
    figlet_text = pyfiglet.figlet_format("AGENT LIFTOFF", font=LOGO_FONT).rstrip("\n")
...
def print_header(console: Console) -> None:
    ...
    console.print(
        Panel(
            "[bold]Welcome to Agent Liftoff[/bold] -- migrate AI agents between orchestration platforms.",
            border_style=AMBER,
            box=box.ROUNDED,
            padding=(0, 2),
        )
...
def print_compact_header(console: Console) -> None:
    ...
    """
    console.print(f"[bold {AMBER}]AGENT_LIFTOFF[/bold {AMBER}]  [dim]-- migrate AI agents between orchestration platforms[/dim]")
...
def print_notes(console: Console) -> None:
    ...
    console.print(
        Panel(
            "[bold]Notes:[/bold]\n\n"
            "1. [bold]Agent Liftoff is early-stage[/bold]\n"
