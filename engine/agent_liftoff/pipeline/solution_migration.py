...
from agent_liftoff.connectors.copilot_studio.mcp_scan import McpServer, find_mcp_servers
from agent_liftoff.connectors.orchestrate.catalog_snapshot import load_snapshot, tools_only
from agent_liftoff.connectors.orchestrate.connections import (
    AppConnection,
    list_applications,
    matching,
)
from agent_liftoff.connectors.orchestrate.exporter import (
    export_agent,
    importable_tools,
    knowledge_base_specs,
    target_model_for,
)
from agent_liftoff.connectors.orchestrate.mcp_sync import McpPlan, plan_servers
from agent_liftoff.foundry import conformance, runtime
from agent_liftoff.foundry.probes.export_scan import _Collected, _scan_tree
from agent_liftoff.foundry.store import FoundryStore
from agent_liftoff.foundry.types import Direction, EntityKind
...
@dataclass
class ManualStep:
    """Something on the target that Agent Liftoff cannot do and a person must.
...
def login_steps(
    ...
    Agent Liftoff never moves credentials -- a solution export does not contain any
...
def ensure_shipped(store: FoundryStore, report: Reporter = silent) -> int:
    """Load the adapters that ship with Agent Liftoff into this machine's store.
    ...
    """
    try:
        from agent_liftoff.assets import ASSETS
        from agent_liftoff.foundry.shipping import install
...
def adapters_ready(
    ...
    if corpus is None:
        return False, (
            f"No stored schema for {platform}. Run `agent_liftoff foundry corridor "
    ...
    if not usable:
        return False, (
            ...
            f"`agent_liftoff foundry corridor {platform} orchestrate` to rebuild."
        )
...
def migrate_solution(
    ...
    *,
    ...
    if client is not None and api_key:
        from agent_liftoff.connectors.orchestrate.adk_session import ensure_session, session_is_live
