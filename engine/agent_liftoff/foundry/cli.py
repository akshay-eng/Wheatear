"""`agent_liftoff foundry` -- probe platforms, compile adapters, run them.
...
from agent_liftoff.foundry import inspector, runtime
from agent_liftoff.foundry.orchestrator import Orchestrator, entity_kinds_in
from agent_liftoff.foundry.probes.base import ProbeContext
from agent_liftoff.foundry.sandbox import DockerSandbox, SubprocessSandbox
...
def _context(platform: str, export: Path | None, url_env: str, key_env: str,
    ...
    return ProbeContext(
        ...
        extra={
            key: value
            for key, value in (("dataverse_token", os.environ.get("AGENT_LIFTOFF_DATAVERSE_TOKEN")),)
            if value
        },
...
def corpora(platform, store_root):
    ...
    if not records:
        click.echo("Nothing probed yet. Run `agent_liftoff foundry probe <platform> <export>`.")
        return
...
def build(platform, direction, entities, fingerprint, rebuild, no_llm, llm_provider, llm_model,
    ...
    if corpus is None:
        raise click.ClickException(
            f"No stored corpus for '{platform}'. Run `agent_liftoff foundry probe {platform} <export>` first."
        )
...
def adapters(platform, store_root):
    ...
    if not found:
        click.echo("No adapters compiled yet. Run `agent_liftoff foundry build <platform>`.")
        return
...
def ship(store_root, destination, platforms):
    ...
    from agent_liftoff.foundry.shipping import ship as ship_assets
