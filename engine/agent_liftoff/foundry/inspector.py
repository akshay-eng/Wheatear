...
from agent_liftoff.foundry.probes.base import MAX_STORED_SAMPLES, ProbeContext, ProbeSource
...
from agent_liftoff.ir.schema import (
    IR_SPEC_VERSION,
    Agent,
    ConnectionRef,
    KnowledgeRef,
...
# Agent Liftoff, not about the source platform.
...
def default_probes(platform: str) -> list[ProbeSource]:
    ...
    from a platform Agent Liftoff has never integrated with still produces a corpus.
    """
    from agent_liftoff.foundry.probes.export_scan import ExportScan

    from agent_liftoff.foundry.probes.write_model import DataverseWriteModel, OrchestrateWriteModel
    ...
    if platform == "orchestrate":
        ...
        from agent_liftoff.foundry.probes.n8n import N8nExportScan
...
def inspect(
    ...
    from agent_liftoff.foundry.conformance import declared_versions  # noqa: PLC0415 - avoids a cycle
...
def ir_corpus() -> SchemaCorpus:
    ...
    return SchemaCorpus(
        ...
        entities=entities,
        gaps=gaps,
        notes=["Read from agent_liftoff.ir.schema; not probed."],
    )
