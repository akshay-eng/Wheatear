...
The Engineer executes code it just generated. That is the one place in Agent Liftoff
...
from agent_liftoff.foundry.types import CaseFailure, SandboxResult
...
MARKER = "<<<AGENT_LIFTOFF-FOUNDRY-RESULT>>>"
...
class DockerSandbox:
    ...
    def run(self, files: dict[str, str]) -> SandboxResult:
        ...
        container = f"agent_liftoff-foundry-{uuid.uuid4().hex[:12]}"
...
class SubprocessSandbox:
    ...
    def run(self, files: dict[str, str]) -> SandboxResult:
        started = time.monotonic()
        with tempfile.TemporaryDirectory(prefix="agent_liftoff-sandbox-") as work:
            root = Path(work)
