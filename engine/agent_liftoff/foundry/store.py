...
Two things are stored, under `~/.local/share/agent_liftoff/foundry` by default:
...
HOME_ENV = "AGENT_LIFTOFF_FOUNDRY_HOME"
...
def default_root() -> Path:
    ...
    XDG data dir rather than the config dir under `~/.config/agent_liftoff`, which
    ...
    root = base / "agent_liftoff" / "foundry"
    # A corridor built before the rename is still a valid corridor -- the
    # fingerprint is taken over platform versions, not the product's name. If
    # the new root is empty and the old one is not, keep using the old one
    # rather than reporting "no adapters compiled on this machine" at somebody
    # whose adapters are sitting right there.
    if not root.exists():
        legacy = base / "wheatear" / "foundry"
        if legacy.exists():
            return legacy
    return root
