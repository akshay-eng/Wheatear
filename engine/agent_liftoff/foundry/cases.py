...
from agent_liftoff.foundry.emit import array_prefix, container_path
from agent_liftoff.foundry.shape import MISSING, resolve_path
...
PROBE_TEXT = "__agent_liftoff_probe__"
...
def derive_cases(
    ...
    if samples:
        ...
        extra["__agent_liftoff_unknown__"] = {"added": ["by", "a", "newer", "platform", "version"]}
        cases.append(
            _case_from_record(
...
HARNESS = '''"""Generated tests for an Agent Liftoff foundry adapter.
...
def render_tests(cases: list[TestCase], spec: MappingSpec) -> str:
    ...
    The harness is generated by Agent Liftoff rather than by a model, even when a
