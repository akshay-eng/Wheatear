...
from agent_liftoff.foundry import guard
from agent_liftoff.foundry.types import AdapterArtifact, EntityKind
...
def load(artifact: AdapterArtifact, verified_only: bool = True) -> LoadedAdapter:
    ...
    module_name = f"agent_liftoff_adapter_{artifact.key.schema_fingerprint[:12]}"
...
def to_ir(entity_kind: EntityKind, payload: dict) -> IRResult:
    ...
    mapping produced something the rest of Agent Liftoff can use", and it is where a
    field the spec got wrong actually surfaces.
    """
    from agent_liftoff.foundry.inspector import IR_MODELS  # noqa: PLC0415 - avoids an import cycle
