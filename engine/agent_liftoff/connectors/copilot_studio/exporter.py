...
from agent_liftoff.errors import ExportError
...
def _solution_xml(unique_name: str) -> str:
    return (
        ...
        'generatedBy="Agent Liftoff" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">\n'
        "  <SolutionManifest>\n"
        ...
        "    <Managed>0</Managed>\n"
        "    <Publisher>\n"
        f"      <UniqueName>{SCHEMA_PREFIX}agent_liftoff</UniqueName>\n"
...
def export_agent(agent: Agent, output_dir: Path) -> ExportResult:
    ...
    unwritable target surfaces as a clear Agent Liftoff error, not a raw traceback.
    """
    try:
