...
Agent Liftoff already holds the API key -- the same key it used to read the tool
...
#
#   agent_liftoff-migration  https://api.us-south.watson-orchestrate.cloud.i…  (active)
#
...
def ensure_session(
    instance_url: str, api_key: str, orchestrate: str, name_hint: str = "agent_liftoff"
) -> str:
    ...
    """
    if not api_key:
        raise LiftoffError("No API key to activate the ADK session with.")
