...
Three YAML shapes are handled:
  ...
  1. REST export (Agent Liftoff's rich format — agent: top-level key):
       agent:
         name: my-agent
         instructions: ...
         llm: ...
  ...
  3. Agent Liftoff's own flat exporter format (spec_version field):
       spec_version: v1
       kind: native
       name: my-agent
       instructions: ...
...
from agent_liftoff.connectors.base import ImportResult, RawKnowledgeRef, RawToolRef
from agent_liftoff.ir.schema import Agent, AgentRef, Guideline
...
def detect_format(path: Path) -> str | None:
    ...
    try:
        ...
        if not isinstance(data, dict):
            return None
        # REST export format (Agent Liftoff rich export)
        ...
        if "orchestrate" in str(data.get("apiVersion", "")).lower():
            return "orchestrate"
        # Agent Liftoff flat exporter format
...
def import_agent(export_path: Path) -> ImportResult:
    ...
    `export_path` may be:
      ...
      - a directory containing agent.yaml (Agent Liftoff exporter layout)
    """
...
def _parse(data: dict) -> ImportResult:
    ...
    # Agent Liftoff flat format
    return _parse_agent_liftoff(data)
...
def _parse_rest_export(data: dict) -> ImportResult:
    """Parse Agent Liftoff's rich REST export format (agent: top-level key)."""
...
def _parse_agent_liftoff(data: dict) -> ImportResult:
    """Parse Agent Liftoff's own Orchestrate exporter output format."""
    ...
    # Tools — list of strings in Agent Liftoff format
    raw_tools = [t for t in data.get("tools", []) or [] if isinstance(t, str) and t]

    # Knowledge bases — stored as "knowledge_base" in Agent Liftoff format
    raw_knowledge = [
    ...
    notes: list[str] = ["Imported from Agent Liftoff Orchestrate export format"]
