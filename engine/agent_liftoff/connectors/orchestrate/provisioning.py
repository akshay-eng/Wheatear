...
their machine, their keyring. Agent Liftoff stores no credential of its own, carries
...
def set_credentials(request: CredentialRequest, secrets: dict[str, str]) -> None:
    ...
    if not secrets:
        raise LiftoffError(f"No credential values supplied for `{request.app_id}`.")
    ...
    if model_name is None:
        raise LiftoffError(f"No credential model for auth kind '{request.kind}'.")
