...
install/refresh/continue loop, used in two places:
  ...
  run_onboarding()        -- the base checks (agent_liftoff.syscheck.check_all),
...
from agent_liftoff.banner import AMBER, SLATE, print_compact_header, print_header, print_notes
from agent_liftoff.config import LiftoffConfig, save_config
from agent_liftoff.syscheck import (
    DependencyStatus,
    check_all,
    check_corridor,
...
def needs_onboarding(config: LiftoffConfig | None) -> bool:
...
def _render_screen(
    ...
    for s in statuses:
        if s.name in restart_pending:
            ...
            # Agent Liftoff restarts -- show that distinctly, never as "missing"
...
def _install_manually(statuses: list[DependencyStatus], restart_pending: dict[str, str]) -> None:
    ...
    console.print(f"  [dim]Opening {shell} -- type 'exit' or Ctrl-D to return to Agent Liftoff.[/dim]")
    subprocess.run([shell])
    console.print("  [dim]Back in Agent Liftoff.[/dim]")
    flush_input()
...
def _show_restart_pending_if_any(restart_pending: dict[str, str]) -> None:
    ...
    console.print(
        Panel(
            ...
            title="[bold cyan]Fixed -- restart Agent Liftoff to activate[/bold cyan]",
            border_style=AMBER,
            expand=False,
        )
    )
...
def run_onboarding(config: LiftoffConfig | None) -> LiftoffConfig:
    ...
    final_config = config or LiftoffConfig()
