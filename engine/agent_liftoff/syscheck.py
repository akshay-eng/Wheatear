...
Two tiers:
  ...
  check_all()      -- base checks Agent Liftoff needs regardless of which
...
def _check_python_version() -> DependencyStatus:
    ...
    return DependencyStatus(
        ...
        required=True,
        # No safe auto-fix: upgrading the interpreter Agent Liftoff itself is
...
def _fix_keyring_backend() -> InstallOutcome:
    ...
    *new* process (confirmed: a fresh interpreter picks it up right away) --
    but re-running the check inside the *same* Agent Liftoff session will still
...
def _keyring_manual_hint() -> str:
    ...
    return (
        ...
        f"(takes effect on Agent Liftoff's *next* restart, not this session)"
    )


def _check_keyring_backend() -> DependencyStatus:
    from agent_liftoff.creds import SERVICE
...
def _dotnet_manual_hint() -> str:
    ...
    return (
        ...
        "add ~/.dotnet to PATH afterward, or restart Agent Liftoff to pick it up)"
    )
