...
with Agent Liftoff's policy of never auto-filling credentials. What IS stored:
...
CONFIG_PATH = Path.home() / ".config" / "agent_liftoff" / "config.json"
# Where the config lived before the product was renamed. Read from, never
# written to: an operator who has already configured their instance URL and
# provider should not have to do it again because the tool changed its name.
LEGACY_CONFIG_PATH = Path.home() / ".config" / "wheatear" / "config.json"
...
def load_config(path: Path = CONFIG_PATH) -> LiftoffConfig | None:
    if not path.exists():
        # Only when the caller did not name a specific file: a test pointing at
        # a temp path means that path, not "fall back to my real config".
...
def save_config(config: LiftoffConfig, path: Path = CONFIG_PATH) -> None:
