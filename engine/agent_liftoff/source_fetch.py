...
into a local directory Agent Liftoff can read.
...
the machine running the wizard, so pointing Agent Liftoff at a URL directly is
...
def clone_repo(repo_url: str, branch: str | None) -> Path:
    ...
    dest = Path(tempfile.mkdtemp(prefix="agent_liftoff-export-"))
