...
Two modes:
  ...
  Manual  -- User provides a path or GitHub URL to an existing Copilot Studio
             export. Agent Liftoff transforms it and writes Orchestrate YAML files.
  ...
  Auto    -- User provides credentials for the source platform (Copilot
             ...
             Agent Liftoff discovers all agents in the environment, the user picks
             which ones to migrate, and Agent Liftoff transforms AND deploys them
...
from agent_liftoff.banner import print_banner, print_compact_header
from agent_liftoff.config import LiftoffConfig, load_config, save_config
from agent_liftoff.onboarding import ensure_corridor_tools, needs_onboarding, run_onboarding
from agent_liftoff.connectors.copilot_studio.importer import detect_format
from agent_liftoff.connectors.copilot_studio.importer import import_agent as copilot_import_agent
from agent_liftoff.connectors.registry import load_exporter
from agent_liftoff.corridors import SOURCE_PLATFORMS, SUPPORTED_CORRIDORS, TARGET_PLATFORMS
from agent_liftoff.eval.generate_cases import generate_cases
from agent_liftoff.llm.factory import PROVIDER_KEY_ENV_DEFAULTS
...
from agent_liftoff.source_fetch import SourceFetchError, resolve_export_source
...
def _prompt_api_key(label: str, keychain_key: str, env_var: str) -> str:
    ...
    """
    from agent_liftoff.creds import load_secret, save_secret
...
def resolve_key_env_for_provider(provider: str, existing: LiftoffConfig | None) -> str:
...
def config_changed(new: LiftoffConfig, old: LiftoffConfig | None) -> bool:
...
def ask_orchestrate_credentials(
    ...
    from agent_liftoff.creds import KEY_TGT_ORCHESTRATE
...
def _provider_label(provider: str) -> str:
    """The LLM provider's name as the product shows it. See `llm/factory.py`."""
...
def ask_llm_settings(
    existing: LiftoffConfig | None, allow_back: bool = False, default: str | None = None
) -> LiftoffConfig:
    console.print(
    ...
    provider = questionary.select(
        "Which LLM provider's key should Agent Liftoff save (for later use)?",
        choices=_with_back_choice(
            [
                questionary.Choice("IBM watsonx", value="ibm-watsonx"),
                questionary.Choice("Anthropic (Claude)", value="anthropic"),
                questionary.Choice("OpenAI", value="openai", disabled="not yet implemented"),
            ],
            allow_back,
        ),
        default=default or "ibm-watsonx",
    ).ask()
    ...
    return LiftoffConfig(llm_provider=provider, llm_key_env=key_env)
...
def _show_migration_plan(
    ...
    table.add_row("LLM provider", _provider_label(config.llm_provider))
...
def _provider_for(config: LiftoffConfig, validate: bool = True):
...
def _run_ai_and_export_stages(
    agent, output_path: Path, target: str, llm_config: LiftoffConfig, provider, llm: str | None = None
) -> Path:
    label = "carrying instructions over (deterministic)" if provider is None else (
        f"synthesizing instructions via {_provider_label(llm_config.llm_provider)}"
    )
...
def _build_final_config(
    llm_config: LiftoffConfig,
    orchestrate_creds: OrchestrateCredentials | None,
    saved_config: LiftoffConfig | None,
...
def ask_orchestrate_source_credentials(
    existing: LiftoffConfig | None = None,
    allow_back: bool = False,
    back_label: str = "◀ Back to previous step",
    default_url: str | None = None,
    ...
    from agent_liftoff.creds import KEY_SRC_ORCHESTRATE
    ...
    from agent_liftoff.config import DEFAULT_WORKSPACE_ID
    saved_ws = (
...
def _maybe_ai_repair_and_retry(name, solution_dir, failed_result, config, deployer) -> bool:
    ...
    proceed = questionary.confirm(
        f"Use {_provider_label(config.llm_provider)} to attempt a fix and retry the push?", default=True
    ).ask()
    ...
    from agent_liftoff.repair import repair_solution

    try:
        provider = build_provider(config.llm_provider, key)
        with console.status(f"  Asking {_provider_label(config.llm_provider)} to fix the solution…"):
...
def _orchestrate_source_wizard(target: str) -> bool:
    ...
    """
    from agent_liftoff.connectors.orchestrate import adk_client as adk
    from agent_liftoff.connectors.orchestrate.importer import import_agent as orch_import_agent
    ...
    while True:
        if step == 1:
            ...
            if target == "orchestrate":
                ...
                from agent_liftoff.connectors.copilot_studio import pac_client as pac
    ...
    export_base = Path(tempfile.mkdtemp(prefix="agent_liftoff-orch-"))
    try:
        ...
        for idx, agent_info in enumerate(ordered_infos, 1):
            ...
            with _make_progress() as prog:
                ...
                try:
                    ...
                    _tlabel = "carrying prompt over" if provider is None else f"{_provider_label(final_config.llm_provider)} AI adapting instructions"
                    prog.update(
                        t,
                    ...
                    if deploy and orchestrate_creds:
                        ...
                        from agent_liftoff.connectors.orchestrate.deployer import deploy_agent
...
def ask_n8n_source_credentials(existing: LiftoffConfig | None) -> N8nSourceCredentials:
    ...
    """
    from agent_liftoff.creds import KEY_N8N_API_KEY
...
def _n8n_source_wizard(target: str) -> bool:
    ...
    from agent_liftoff.connectors.n8n import importer as n8n_importer
    from agent_liftoff.connectors.n8n import n8n_client
    ...
    if selected_names is not None:
        from agent_liftoff.workflow import reachable_ids
    ...
    provider = _provider_for(existing or LiftoffConfig())
    ...
    for agent in ordered:
        ...
        try:
            ...
            _run_ai_and_export_stages(agent, out_dir, target, existing or LiftoffConfig(), provider, llm=llm)
        except SystemExit:
            raise
        except Exception as exc:  # noqa: BLE001
    ...
    from agent_liftoff.connectors.orchestrate.provisioner import provision_and_deploy
    ...
    desc_provider = _corridor_provider(existing or LiftoffConfig())
    ...
    from agent_liftoff.reporting import facts_from_deploy_reports
...
def _copilot_studio_export_wizard(target: str) -> bool:
    ...
    console.print(
        Panel(
            "Point Agent Liftoff at a Copilot Studio solution export — either the [bold].zip[/bold]\n"
            "you downloaded, or a folder you already unpacked it into.\n\n"
            "A managed or unmanaged export both work. The folder Agent Liftoff needs is the one\n"
...
def _write_reports(facts, destination: Path) -> None:
    ...
    from agent_liftoff.reporting import write_all
...
def _finish_migration(report, orchestrate_creds, output_base: Path, source_platform: str) -> None:
    ...
    from agent_liftoff.reporting import facts_from_solution_report
...
def _run_export_solution_migration(
    ...
    from agent_liftoff.pipeline.solution_migration import migrate_solution
...
def _copilot_studio_auto_wizard(source: str, target: str) -> bool:
    ...
    scan_base = Path(tempfile.mkdtemp(prefix="agent_liftoff-scan-"))
    ...
    try:
        ...
        from agent_liftoff.pipeline.solution_migration import adapters_ready
...
def _validate_llm_key(config: LiftoffConfig) -> None:
    """Lightweight auth check against the LLM provider (lists models, no tokens used)."""
    from agent_liftoff.llm.factory import validate_api_key
    key = os.environ.get(config.llm_key_env, "")
    try:
        with console.status(f"  Validating {_provider_label(config.llm_provider)} API key…"):
            validate_api_key(config.llm_provider, key)
        console.print(f"  [green]✓[/green]  {_provider_label(config.llm_provider)} API key accepted")
        ...
        console.print(
            f"  [yellow]⚠[/yellow]  Could not reach {_provider_label(config.llm_provider)} to validate key "
...
def _corridor_provider(config: LiftoffConfig):
    ...
    from agent_liftoff.creds import llm_key_name, load_secret
    from agent_liftoff.llm.factory import build_provider

    key = os.environ.get(config.llm_key_env) or load_secret(llm_key_name(config.llm_provider))
    if not key:
        console.print(
            f"  [yellow]No {_provider_label(config.llm_provider)} key found.[/yellow] Tool matching will "
...
def _observed(provider):
    ...
    """
    global _METER
    from agent_liftoff.llm.usage import ObservedProvider, UsageMeter
...
def _foundry_store(saved_config: LiftoffConfig | None):
...
def _ask_store_root(store, reason: str):
    ...
    console.print(
        Panel(
            ...
            "  [cyan]agent_liftoff foundry probe copilot-studio --export <unpacked-solution> --offline[/cyan]\n"
            "  [cyan]agent_liftoff foundry probe orchestrate[/cyan]\n"
            "  [cyan]agent_liftoff foundry corridor copilot-studio orchestrate[/cyan]\n\n"
            ...
            "  Already built elsewhere? Point Agent Liftoff at that directory below.",
...
def _show_manual_steps(report) -> None:
    """The things Agent Liftoff cannot do and a person must, printed to be acted on.
...
def _console_origin(instance_url: str) -> str:
    try:
        from agent_liftoff.connectors.orchestrate.catalog_client import console_origin
...
def _ask_credentials(request) -> dict[str, str] | None:
    ...
    written to an Agent Liftoff file, not put in a review manifest, not logged, and
    ...
    console.print(
        Panel(
            ...
            "your own OS keychain if you say so. Agent Liftoff stores nothing itself and "
    ...
    if questionary.confirm("Save these to your OS keychain for next time?", default=False).ask():
        ...
        console.print("  [dim]Saved to your keychain. Nothing left Agent Liftoff's process.[/dim]")
    return secrets
...
def _ask_auth_kind(app_id: str, offered: tuple[str, ...]) -> str | None:
    ...
    """
    from agent_liftoff.connectors.orchestrate.provisioning import CREDENTIAL_FIELDS
...
def _ask_server_url(request) -> bool:
    ...
    """
    from agent_liftoff.connectors.orchestrate.provisioning import existing_server_url, looks_like_a_url
...
def _ask_console_cookie(instance_url: str) -> str | None:
    ...
    console.print(
        Panel(
            "Agent Liftoff can install these for you, but the catalog only accepts a browser\n"
...
def _auto_install(pending: list, instance_url: str, cookie: str) -> list[str]:
    ...
    from agent_liftoff.connectors.orchestrate.catalog_install import ConsoleSession, install_artifact
...
def _setup_connections(report, client, instance_url: str, token: str) -> None:
    ...
    """
    from agent_liftoff.connectors.orchestrate.connections import bound_connection, list_applications
    from agent_liftoff.connectors.orchestrate.provisioning import CredentialRequest
...
def _show_model_choice(agents: list, chosen: str | None, fallback_note: str, live: bool) -> None:
    ...
    if not chosen:
        return
    from agent_liftoff.pipeline.solution_migration import _model_reasoning
...
def _connection_manual_step(app_id: str, tools: list[str], current) -> object:
    ...
    from agent_liftoff.pipeline.solution_migration import CONNECTIONS_ROUTE, ManualStep
...
def _watch_for_installs(report, client, instance_url: str, rerun):
    ...
    console.
    """
    from agent_liftoff.pipeline.solution_migration import installed_tool_names, still_missing
...
def _run_foundry_migration(
    ...
    """
    from agent_liftoff.pipeline.solution_migration import MigrationReport, migrate_solution
